...
const pagStates = {};
function initPag(id, data, renderFn, tbodyId) {
...
window.pagGo = function pagGo(id, delta) {
    const st = pagStates[id];
    if (!st)
        return;
...
document.getElementById('btnLogout').addEventListener('click', () => {
    ...
    token = null;
    showView('login');
    Object.values(charts).forEach(c => { try {
        c?.destroy();
    }
    catch { } });
    charts = {};
...
function actualizarDashboardResumen() {
...
async function loadHerramientas() {
    try {
        herrData = (await apiFetch('/herramientas')) || [];
...
async function loadStatsUPS() {
    try {
        const stats = await apiFetch('/stats/ups');
        if (stats)
            renderChartsUPS(stats);
    }
    catch (err) {
        toast(err.message, 'error');
    }
}
async function loadStatsHerr() {
    try {
        const stats = await apiFetch('/stats/herramientas');
        if (stats)
            renderChartsHerr(stats);
    }
    catch (err) {
        toast(err.message, 'error');
    }
}
function updateResumenUPS() {
    const counts = {};
    upsData.forEach(u => { counts[u.estado] = (counts[u.estado] || 0) + 1; });
    Object.keys(estadoLabelsUPS).forEach(e => {
        const el = document.getElementById(`ups-count-${e}`);
        if (el)
            el.textContent = String(counts[e] || 0);
    });
}
function herramientaEnUso(h) {
    return proyectosData.some(p => p.herramientas_asignadas?.split('||').some(e => e.startsWith(String(h.id) + '::')));
}
function updateResumenHerr() {
...
async function loadStatsProyectos() {
    try {
        const stats = await apiFetch('/stats/proyectos');
        if (stats)
            renderChartsProyectos(stats);
    }
    catch (err) {
        toast(err.message, 'error');
    }
}
function renderChartsProyectos(stats) {
    ['proyEstado', 'proyHerr', 'proyMeses', 'proyResponsable'].forEach(k => { try {
        charts[k]?.destroy();
    }
    catch { } ; delete charts[k]; });
    const emptyLabel = 'Sin datos';
    const emptyColor = '#e2e8f0';
    const meses = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'];
    const ctxEstado = document.getElementById('chart-proy-estado').getContext('2d');
    ...
    const ctxHerr = document.getElementById('chart-proy-herr').getContext('2d');
    ...
    const ctxMeses = document.getElementById('chart-proy-meses').getContext('2d');
    ...
    const ctxResp = document.getElementById('chart-proy-responsable').getContext('2d');
...
function renderTableUPS() {
    const input = document.querySelector('.filtro-buscar[data-section="ups"]');
    const select = document.querySelector('.filtro-estado[data-section="ups"]');
...
function renderTableUPSPage(filtered) {
      ...
      <tr>
        ...
        <td>
          <select class="estado-select" data-id="${u.id}" data-section="ups" style="padding:4px 6px;border:1px solid var(--border);border-radius:6px;font-size:0.78rem;background:var(--surface);color:var(--text);cursor:pointer;">
            ${Object.entries(estadoLabelsUPS).map(([val, lbl]) => `<option value="${val}" ${u.estado === val ? 'selected' : ''}>${lbl}</option>`).join('')}
          </select>
        </td>
...
function renderTableHerr() {
    const input = document.querySelector('.filtro-buscar[data-section="herramientas"]');
    const select = document.querySelector('.filtro-estado[data-section="herramientas"]');
    const proyFilter = document.getElementById('filtro-proyecto-herr')?.value || '';
    const text = (input?.value || '').toLowerCase();
    const filter = select?.value || '';
    const filtered = herrData.filter(h => {
        ...
        if (proyFilter) {
            const enProy = proyectosData.some(p => String(p.id) === proyFilter &&
...
function renderTableHerrPage(filtered) {
...
async function verHistorial(id) {
    try {
        const historial = await apiFetch(`/herramientas/${id}/historial`);
...
function renderChartsUPS(stats) {
    ['upsEstado', 'upsModelo', 'upsMeses', 'upsUbicacion'].forEach(k => { try {
        charts[k]?.destroy();
    }
    catch { } ; delete charts[k]; });
    const emptyLabel = 'Sin datos';
    const emptyColor = '#e2e8f0';
    const ctxEstado = document.getElementById('chart-ups-estado').getContext('2d');
    ...
    const ctxModelo = document.getElementById('chart-ups-modelo').getContext('2d');
    ...
    const meses = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'];
    const ctxMeses = document.getElementById('chart-ups-meses').getContext('2d');
    ...
    const ctxUbic = document.getElementById('chart-ups-ubicacion').getContext('2d');
...
function renderChartsHerr(stats) {
    ['herrEstado', 'herrMarca', 'herrMeses', 'herrUbicacion'].forEach(k => { try {
        charts[k]?.destroy();
    }
    catch { } ; delete charts[k]; });
    const emptyLabel = 'Sin datos';
    const emptyColor = '#e2e8f0';
    const ctxEstado = document.getElementById('chart-herr-estado').getContext('2d');
    ...
    const ctxMarca = document.getElementById('chart-herr-marca').getContext('2d');
    ...
    const meses = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic'];
    const ctxMeses = document.getElementById('chart-herr-meses').getContext('2d');
    ...
    const ctxUbic = document.getElementById('chart-herr-ubicacion').getContext('2d');
    charts.herrUbicacion = new Chart(ctxUbic, {
        type: 'bar', data: { labels: stats.ubicaciones?.length ? stats.ubicaciones.map(u => u.ubicacion) : [emptyLabel], datasets: [{ label: 'Cantidad', data: stats.ubicaciones?.length ? stats.ubicaciones.map(u => u.count) : [0], backgroundColor: stats.ubicaciones?.length ? '#2563eb' : emptyColor, borderRadius: 6 }] },
        options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { y: { ticks: { stepSize: 1 }, grid: { display: false } }, x: { grid: { display: false } } } },
    });
}
async function openFormUPS(id = null) {
    if (id !== null && !await confirmarAdmin())
        return;
    const modal = document.getElementById('modal-ups');
    modal.classList.add('active');
    document.getElementById('form-ups').reset();
    document.getElementById('ups-id').value = '';
    if (id) {
        const u = upsData.find(x => x.id === id);
        if (!u)
            return;
        document.getElementById('modal-ups-title').textContent = 'Editar UPS';
        document.getElementById('ups-id').value = String(u.id);
        document.getElementById('ups-serial').value = u.serial_number;
        document.getElementById('ups-modelo').value = u.modelo || '';
        document.getElementById('ups-capacidad').value = u.capacidad || '';
        document.getElementById('ups-ubicacion').value = u.ubicacion || '';
        document.getElementById('ups-estado').value = u.estado;
        document.getElementById('ups-fecha_ingreso').value = u.fecha_ingreso || '';
        document.getElementById('ups-fecha_salida').value = u.fecha_salida || '';
        document.getElementById('ups-notas').value = u.notas || '';
    }
    else {
        document.getElementById('modal-ups-title').textContent = 'Nueva UPS';
        document.getElementById('ups-estado').value = 'activa';
        document.getElementById('ups-fecha_ingreso').value = new Date().toISOString().split('T')[0];
    }
}
async function deleteUPS(id) {
    if (!await confirmarAdmin())
        return;
    if (!confirm('¿Eliminar esta UPS?'))
        return;
    try {
...
async function confirmarAdmin() {
    return new Promise(resolve => {
        ...
        const verificar = async () => {
            ...
            try {
                const res = await apiFetch('/auth/verify-password', { method: 'POST', body: JSON.stringify({ password: pw }) });
                if (res?.ok === true) {
                    limpiar();
                    resolve(true);
                }
        ...
        btnConfirmar.onclick = verificar;
        btnCancelar.onclick = cancelar;
        btnCerrar.onclick = cancelar;
        input.onkeydown = (e) => { if (e.key === 'Enter')
            verificar(); if (e.key === 'Escape')
            cancelar(); };
        overlay.classList.add('active');
        setTimeout(() => input.focus(), 100);
    });
}
async function openFormHerr(id = null) {
    if (id !== null && !await confirmarAdmin())
        return;
    const modal = document.getElementById('modal-herramientas');
    modal.classList.add('active');
    document.getElementById('form-herramientas').reset();
    document.getElementById('herr-id').value = '';
    if (id) {
        const h = herrData.find(x => x.id === id);
        if (!h)
            return;
        document.getElementById('modal-herr-title').textContent = 'Editar Herramienta';
        document.getElementById('herr-id').value = String(h.id);
        document.getElementById('herr-codigo').value = h.codigo;
        document.getElementById('herr-nombre').value = h.nombre || '';
        document.getElementById('herr-marca').value = h.marca || '';
        document.getElementById('herr-modelo').value = h.modelo || '';
        document.getElementById('herr-ubicacion').value = h.ubicacion || '';
        document.getElementById('herr-estado').value = h.estado;
        document.getElementById('herr-fecha').value = h.fecha_adquisicion || '';
        document.getElementById('herr-notas').value = h.notas || '';
    }
    else {
        document.getElementById('modal-herr-title').textContent = 'Nueva Herramienta';
        document.getElementById('herr-estado').value = 'disponible';
        document.getElementById('herr-fecha').value = new Date().toISOString().split('T')[0];
    }
}
async function deleteHerr(id) {
    if (!await confirmarAdmin())
        return;
...
async function loadAuditoria() {
    const tabla = document.getElementById('audit-filtro').value;
    try {
        const logs = await apiFetch(`/auditoria${tabla ? '?tabla=' + tabla : ''}`);
        const tbody = document.getElementById('audit-tabla-body');
        if (!logs || logs.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:30px;color:#94a3b8;">Sin registros</td></tr>';
            return;
        }
...
async function loadProyectos() {
    try {
        proyectosData = (await apiFetch('/proyectos')) || [];
        renderProyectos();
...
async function loadMovimientos() {
    try {
        movimientosData = (await apiFetch('/movimientos')) || [];
...
function renderProyectosPage(filtered) {
...
function renderMovimientos() {
    const filtroProyecto = document.getElementById('filtro-proyecto-mov')?.value || '';
...
async function actualizarSelectProyectos() {
    const selects = ['filtro-proyecto-mov', 'asignar-proyecto'].map(id => document.getElementById(id));
...
async function actualizarSelectProyectoHerr() {
    const sel = document.getElementById('filtro-proyecto-herr');
    if (!sel)
        return;
    const val = sel.value;
...
async function abrirFormProyecto(id = null) {
    if (id !== null && !await confirmarAdmin())
        return;
    ...
    if (id) {
        ...
        document.getElementById('proyecto-id').value = String(p.id);
        document.getElementById('proy-nombre').value = p.nombre;
        document.getElementById('proy-ubicacion').value = p.ubicacion || '';
        document.getElementById('proy-fecha_inicio').value = p.fecha_inicio || '';
        document.getElementById('proy-fecha_fin').value = p.fecha_fin || '';
        ...
        if (p.herramientas_asignadas) {
            p.herramientas_asignadas.split('||').forEach(entry => {
                ...
                const cb = document.getElementById(`herr-cb-${idHerr}`);
                if (cb)
                    cb.checked = true;
            });
        }
...
window.openFormUPS = openFormUPS;
window.deleteUPS = deleteUPS;
window.openFormHerr = openFormHerr;
window.deleteHerr = deleteHerr;
window.asignarHerramienta = asignarHerramienta;
window.editarProyecto = abrirFormProyecto;
window.eliminarProyecto = eliminarProyecto;
window.cerrarProyecto = cerrarProyecto;
window.cambiarResponsable = cambiarResponsable;
async function cargarSelectorHerramientas(proyectoActualId = null) {
    const container = document.getElementById('proy-herr-selector');
    try {
        const toolProyectoMap = {};
        for (const p of proyectosData) {
            if (p.id === proyectoActualId)
                continue;
            if (p.herramientas_asignadas) {
                p.herramientas_asignadas.split('||').forEach(entry => {
                    const [idStr] = entry.split('::');
                    const tid = parseInt(idStr, 10);
                    if (!isNaN(tid) && !toolProyectoMap[tid])
                        toolProyectoMap[tid] = p.nombre;
                });
            }
        }
        const herramientas = (await apiFetch('/herramientas')) || [];
        ...
        container.innerHTML = '<p style="color:#ef4444;padding:8px;">Error al cargar herramientas</p>';
    }
}
async function eliminarProyecto(id) {
    if (!await confirmarAdmin())
        return;
    if (!confirm('¿Eliminar este proyecto? También se eliminarán sus movimientos.'))
        return;
    try {
...
async function cerrarProyecto(id) {
    if (!await confirmarAdmin())
        return;
    if (!confirm('¿Cerrar este proyecto? Se liberarán todas las herramientas asignadas.'))
        return;
    try {
...
async function cambiarResponsable(id) {
    const p = proyectosData.find(x => x.id === id);
    if (!p)
        return;
...
async function asignarHerramienta(id) {
    ...
    if (!h)
        return;
    document.getElementById('asignar-herramienta-id').value = String(h.id);
    document.getElementById('asignar-herramienta-nombre').value = `${h.codigo} - ${h.nombre}`;
    document.getElementById('asignar-fecha').value = new Date().toISOString().split('T')[0];
    document.getElementById('asignar-proyecto').value = '';
    document.getElementById('asignar-recibio').value = '';
    document.getElementById('asignar-notas').value = '';
...
async function generarReporte() {
    try {
        const data = await apiFetch('/reporte/movimientos');
        if (!data || !data.length) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.btn-export').forEach(btn => {
        btn.addEventListener('click', () => exportExcel(btn.dataset.section));
    });
    document.querySelectorAll('.btn-pdf').forEach(btn => {
        btn.addEventListener('click', () => exportPDF(btn.dataset.section));
    });
    document.querySelectorAll('.modal-close, .btn-secondary[data-modal]').forEach(el => {
        el.addEventListener('click', closeAllModals);
    });
    document.querySelectorAll('.modal-overlay').forEach(m => {
        m.addEventListener('click', (e) => { if (e.target === e.currentTarget)
            closeAllModals(); });
    });
    ...
    document.getElementById('btnPerfil').addEventListener('click', () => {
        ...
        document.getElementById('perfil-rol').value = usuarioActual?.rol || '';
    ...
    document.getElementById('btnRestore').addEventListener('click', () => {
        ...
        if (!fileInput.files?.[0])
            return toast('Seleccioná un archivo', 'error');
    ...
    document.getElementById('form-proyecto').addEventListener('submit', async (e) => {
        ...
        const herramientasSeleccionadas = Array.from(document.querySelectorAll('#proy-herr-selector input[type="checkbox"]:checked')).map(cb => Number(cb.value));
        const movidas = [];
        for (const hId of herramientasSeleccionadas) {
            for (const p of proyectosData) {
                ...
                if (p.herramientas_asignadas?.split('||').some(e => e.startsWith(String(hId) + '::'))) {
                    ...
                    if (h)
                        movidas.push(`${h.codigo} (desde: ${p.nombre})`);
                    break;
                }
            }
        }
        ...
        const data = {
            ...
            fecha_inicio: document.getElementById('proy-fecha_inicio').value || null,
            fecha_fin: document.getElementById('proy-fecha_fin').value || null,
            responsable: document.getElementById('proy-responsable').value.trim(),
    ...
    document.getElementById('btn-select-all-herr').addEventListener('click', () => {
        document.querySelectorAll('#proy-herr-selector input[type="checkbox"]').forEach(cb => cb.checked = true);
    });
    document.getElementById('btn-deselect-all-herr').addEventListener('click', () => {
        document.querySelectorAll('#proy-herr-selector input[type="checkbox"]').forEach(cb => cb.checked = false);
    });
    document.getElementById('form-asignar').addEventListener('submit', async (e) => {
        ...
        setLoading(btn, true);
        const data = {
            herramienta_id: Number(document.getElementById('asignar-herramienta-id').value),
            proyecto_id: Number(document.getElementById('asignar-proyecto').value),
            fecha_salida: document.getElementById('asignar-fecha').value || null,
            recibido_por: document.getElementById('asignar-recibio').value.trim(),
            notas: document.getElementById('asignar-notas').value.trim(),
        };
        try {
    ...
    document.getElementById('form-password').addEventListener('submit', async (e) => {
        ...
        try {
            await apiFetch('/auth/password', { method: 'PUT', body: JSON.stringify({
                    currentPassword: document.getElementById('perfil-current').value,
                    newPassword: document.getElementById('perfil-new').value,
                }) });
    ...
    document.getElementById('form-ups').addEventListener('submit', async (e) => {
        ...
        const btn = e.target.querySelector('button[type="submit"]');
        setLoading(btn, true);
        const id = document.getElementById('ups-id').value;
        const data = {
            serial_number: document.getElementById('ups-serial').value.trim(),
            modelo: document.getElementById('ups-modelo').value.trim(),
            capacidad: document.getElementById('ups-capacidad').value.trim(),
            ubicacion: document.getElementById('ups-ubicacion').value.trim(),
            estado: document.getElementById('ups-estado').value,
            fecha_ingreso: document.getElementById('ups-fecha_ingreso').value || null,
            fecha_salida: document.getElementById('ups-fecha_salida').value || null,
            notas: document.getElementById('ups-notas').value.trim(),
        };
        try {
    ...
    document.getElementById('form-herramientas').addEventListener('submit', async (e) => {
        ...
        const btn = e.target.querySelector('button[type="submit"]');
        setLoading(btn, true);
        const id = document.getElementById('herr-id').value;
        const data = {
            codigo: document.getElementById('herr-codigo').value.trim(),
            nombre: document.getElementById('herr-nombre').value.trim(),
            marca: document.getElementById('herr-marca').value.trim(),
            modelo: document.getElementById('herr-modelo').value.trim(),
            ubicacion: document.getElementById('herr-ubicacion').value.trim(),
            estado: document.getElementById('herr-estado').value,
            fecha_adquisicion: document.getElementById('herr-fecha').value || null,
            notas: document.getElementById('herr-notas').value.trim(),
        };
        try {
    ...
    document.querySelectorAll('.filtro-buscar').forEach(el => {
        el.addEventListener('input', () => {
            if (el.dataset.section === 'ups')
                renderTableUPS();
            else
                renderTableHerr();
        });
